...
const int Chromosome_length = 6;    //个体数为64，用6位表示每一个个体
double rate_crossover = 0.5;                //交叉率
double rate_mutation = 0.001;           //变异率
int iteration_num = 50;                     //初始设定进化50代                                     
/****************************************/
...
void main()                                   
{
    ...
    printf("遗传算法求函数 y = -(x-1)^2 + 5 的最大值应用如下\n");

enter:printf("请输入您要设定的迭代次数 : ");
...
// 函数：将二进制换算为十进制 
void decode(chromosome &population_current)   
{//此处的染色体长度为6，其中第6个表示符号位  
    int i = 0;
    population_current.value = 0;
    //低位在前，高位再后
    for( i = Chromosome_length -1 ; i > 0; i-- ) 
        population_current.value += (double)pow(2,Chromosome_length-i-1) * (double)population_current.bit[i];    //遍历染色体二进制编码, 
    //最高位为符号位，如果是1代表负数
